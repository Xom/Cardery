 ...
 */
$(()=>{
  console && console.log('Cardery v1.0.0');
  class Cardery {
    ...
    async run () {try {
      $('head').append('<style>body{-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale;}\
        #previewDiv img{height:75vh;width:auto;}</style>');
    ...
    async render (card, force) {
      if (!card.dataUrl || force) {
        this.mainDiv.empty();
        ...
        this.zip.file(`${this.basenames[card.csv]}.${card.i}.png`, card.dataUrl.substr(card.dataUrl.indexOf(',') + 1), {base64: true});
  ...
  function showError(err) {
    ...
    $('body').prepend(`<p style="color:#c00;">Error (open console for more info):<br>${err}<br>${JSON.stringify(obj, null, ' ')}</p>`.replace(/\\n/g, '<br>'));
  }
